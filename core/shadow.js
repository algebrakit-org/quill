...
function internalGetShadowSelection(root) {
  ...
  if (rightNode === null) {
    ...
    textRightNode.insertData(
      textRightNode.length,
      rightText.substr(textRightNode.length),
    );
  ...
  if (leftNode.nodeType === Node.TEXT_NODE) {
    ...
    const start =
      textLeftNode === rightNode ? rightOffset : leftText.length - 1;
    ...
    textLeftNode.insertData(
      textLeftNode.length,
      leftText.substr(textLeftNode.length),
    );
